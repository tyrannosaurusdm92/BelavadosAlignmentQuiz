...
(function(){
  ...
  function respond(loreInput, command, state){
    ...
    if(intent === 'settlement'){
      const settlement = generateSettlement(lore, {command, npcCap: state && state.npcCap || 350});
      return {intent, message:`Generated ${settlement.name}, a ${settlement.danger} ${settlement.type} in ${settlement.province}, with ${settlement.locations.length} visitable locations and ${settlement.npcs.length} NPC records.`, data:settlement};
    }
    if(intent === 'npcs'){
      const result = generateNPCBatch(lore, {command, settlement: current && current.schema && current.schema.includes('settlement') ? current : null, count: extractCount(command, 25)});
      return {intent, message:`Generated ${result.npcs.length} NPCs${result.settlement ? ' for '+result.settlement : ''}.`, data:result};
    }
    if(intent === 'province'){
      const result = generateProvince(lore, {command, fast:true, npcCap: state && state.npcCap || 140});
      return {intent, message:`Generated ${result.settlementCount} settlements for ${result.province}.`, data:result};
    }
    if(intent === 'search'){
      const found = searchState(command, current);
      return {intent, message: found.message, data:found.data};
    }
    if(intent === 'export') return {intent, message:'Use the Export JSON or Export Settlement HTML buttons. I prepared the current data for download.', data: current || null};
    return {intent, message: casualReply(lore, command, current), data:null};
  }
  function helpText(){
    return [
      'Try commands like:',
      '• create a high danger deep mountain mining town in Kharvathen named Coalveil with train and regulated portal, population 4200',
      '• generate 40 NPCs for this settlement',
      '• generate province Aelvanyr with 3 villages 2 towns 1 city',
      '• find taverns / show portal / search doctor',
      '',
      'This is a rule-based bot, not a true AI model. It follows lore data, keyword parsing, weighted tables, and generator rules.'
    ].join('\n');
  }
  ...
  function exportSettlementHtml(settlement){
    const safe = toPlayerSafe(settlement);
    const title = `${safe.name} - Belavadös Settlement`;
    const data = JSON.stringify(safe).replace(/</g,'\\u003c');
    return `<!DOCTYPE html>\n<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><style>body{margin:0;background:#07111d;color:#edf8ff;font-family:system-ui,Segoe UI,Arial,sans-serif}main{width:min(1100px,calc(100% - 24px));margin:0 auto;padding:20px}.card{border:1px solid #ffffff22;background:#0b1b2d;border-radius:16px;padding:14px;margin:12px 0}h1,h2{color:#dffcff}.pill{display:inline-block;border:1px solid #65d8ff55;border-radius:999px;padding:4px 8px;margin:3px;color:#dffcff}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:10px}</style></head><body><main><h1>${escapeHtml(safe.name)}</h1><p>${escapeHtml(safe.publicSummary)}</p><div id="app"></div><script>const SETTLEMENT=${data};function e(s){return String(s??'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]))}document.getElementById('app').innerHTML='<section class="card"><h2>Overview</h2><p><b>Province:</b> '+e(SETTLEMENT.province)+' · <b>Type:</b> '+e(SETTLEMENT.type)+' · <b>Biome:</b> '+e(SETTLEMENT.biome)+' · <b>Population:</b> '+e(SETTLEMENT.population)+'</p><p>'+SETTLEMENT.transport.map(t=>'<span class="pill">'+e(t)+'</span>').join('')+'</p></section><section class="card"><h2>Locations</h2><div class="grid">'+SETTLEMENT.locations.map(l=>'<article class="card"><h3>'+e(l.name)+'</h3><p>'+e(l.type)+' · '+e(l.category)+'</p><p>'+e(l.description)+'</p><p><b>Services:</b> '+e((l.services||[]).join(', '))+'</p></article>').join('')+'</div></section><section class="card"><h2>Public NPC Sample</h2><div class="grid">'+SETTLEMENT.npcs.slice(0,80).map(n=>'<article class="card"><h3>'+e(n.fullName)+'</h3><p>'+e(n.race)+' · '+e(n.genderIdentity)+' · '+e(n.occupation)+'</p><p>'+e(n.publicBio)+'</p></article>').join('')+'</div></section>'<\/script></main></body></html>`;
  }
  ...
  RuleBot.generateSettlement = generateSettlement;
  RuleBot.generateNPCBatch = generateNPCBatch;
  RuleBot.generateProvince = generateProvince;
  RuleBot.parseIntent = parseIntent;
  RuleBot.toPlayerSafe = toPlayerSafe;
  RuleBot.exportSettlementHtml = exportSettlementHtml;
