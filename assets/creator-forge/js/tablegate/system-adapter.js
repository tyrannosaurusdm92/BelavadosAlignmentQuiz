(function (global) {
  ...
  function populateProject(state=LS.store.get()) {
    ...
    if(summary && p.item && p.edition){
      summary.innerHTML=`<strong>${esc(p.item.name)} · ${esc(p.edition.label)}</strong><span>${p.edition.ancestries.length} ${esc(p.edition.identityLabel.toLowerCase())} options · ${p.edition.roles.length} ${esc(p.edition.roleLabel.toLowerCase())} options · ${p.edition.specializations.length} specialties · ${p.edition.abilities.length} abilities/skills</span><small>Full source: json/systems/${esc(p.item.source)}</small>`;
    }
  }
