...
(function(){
  ...
  LS.generateLocations = (config) => {
    ...
    // Guarantee core support and transit types from settlement rules.
    const required = [];
    const has = (rx) => out.some(l=>rx.test(`${l.name} ${l.type} ${l.category}`));
    const addRequired = (base) => {
      if(has(new RegExp(base.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'i'))) return;
      const category = LS.categorizeLocation(base);
      const name = LS.generateLocationName(base, used, config);
      const loc = {id:LS.uid('loc'), name, type:base, category, biome:LS.choose(config.biomeStack||[],{}), district:'Civic Core', ownership:'civic-chartered', hours:'always staffed', reputation:'essential', description:LS.locationDescription(name,category,config), rumor:'Everyone knows someone who works here.', secret:'The records are incomplete.', plotHook:'A missing record changes a family, transit, or faction claim.', validationNote:'', residents:[], employees:[], visitors:[], services:[]};
      loc.services = LS.generateServiceItems(loc, config, 4); out.push(loc); required.push(base);
    };
    const biomeText=(config.biomeStack||[]).map(b=>b.option).join(' ');
    const underwater=/underwater|reef/i.test(biomeText);
    const water=/ocean|water|river|reef|beach|coast|marsh|swamp/i.test(biomeText);
    const cityOrCapital=/City|Capital/i.test(config.settlementSize);
    if(!has(/house|apartment|residence|boarding|tenement|rowhouse|loft|quarters|barracks|dwelling/i)) addRequired('Apartment row');
    if(!has(/hospital|healer|clinic|apothecary|medicine/i)) addRequired('Healer clinic');
    if(!has(/tavern|inn|restaurant|market|bakery|food|farm|granary/i)) addRequired('Tavern');
    if(!underwater){ if(!has(/train|rail/i)) addRequired('Train Station'); if(!has(/caravan/i)) addRequired('Caravan Station'); }
    if(water && !has(/ferry/i)) addRequired('Ferry Terminal');
    if(water && cityOrCapital && !has(/steamship/i)) addRequired('Steamship Port');
    if(underwater && !has(/submarine/i)) addRequired('Submarine Terminal');
    if(cityOrCapital && !underwater && !has(/skyship/i)) addRequired('Skyship Port');
    if(/Capital/i.test(config.settlementSize) && !has(/portal/i)) addRequired('Portal Facility');
