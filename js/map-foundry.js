...
function generateSettlement(map,rng){
 const walls=blobPoints(rng,500,330,365,250,24,.15),center=[500,330],wardCount=scopeCounts('settlement',map.detail),wards=[],roads=[],buildings=[],labels=[],gates=[];
 const angles=Array.from({length:wardCount},(_,i)=>Math.PI*2*i/wardCount+(rng.next()-.5)*.12);
 for(let i=0;i<wardCount;i++){
  const a1=angles[i],a2=angles[(i+1)%wardCount]||(Math.PI*2+angles[0]),inner=rng.int(35,85),outer=rng.int(210,330),p1=polar(...center,inner,a1),p2=polar(...center,outer,a1),pm=polar(...center,outer+rng.int(-25,25),(a1+a2)/2),p3=polar(...center,outer,a2),p4=polar(...center,inner,a2),name=Names.make(rng),pts=[p1,p2,pm,p3,p4];wards.push({name,points:pts});labels.push(`<text x="${polar(...center,(inner+outer)*.55,(a1+a2)/2)[0].toFixed(0)}" y="${polar(...center,(inner+outer)*.55,(a1+a2)/2)[1].toFixed(0)}" text-anchor="middle">${escAttr(name)}</text>`);roads.push(`<path d="M500,330 L${p2[0].toFixed(0)},${p2[1].toFixed(0)}"/>`);
  for(let b=0;b<rng.int(8,18);b++){const a=a1+rng.next()*(a2-a1),r=inner+20+rng.next()*(outer-inner-45),[x,y]=polar(...center,r,a),w=rng.int(8,18),h=rng.int(6,14),rot=Math.round(a*180/Math.PI+90);buildings.push(`<rect x="${(x-w/2).toFixed(1)}" y="${(y-h/2).toFixed(1)}" width="${w}" height="${h}" rx="1" transform="rotate(${rot} ${x.toFixed(1)} ${y.toFixed(1)})"/>`)}
 ...
 const svg=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 650"><rect width="1000" height="650" fill="#b4a875"/><path d="M0,90 Q240,40 430,100 T1000,85 V0 H0Z" fill="#4f758c" opacity=".85"/><path d="${smoothPath(walls)}" fill="#d8c995" stroke="#4e4433" stroke-width="14"/>${wardSvg}<g class="roads">${roads.join('')}</g><g class="buildings">${buildings.join('')}</g><circle cx="500" cy="330" r="66" fill="#d2b96c" stroke="#5b4024" stroke-width="5"/><rect x="468" y="298" width="64" height="64" fill="#8a6341" stroke="#3e2a1c" stroke-width="4"/><g class="gates">${gates.join('')}</g><g class="labels">${labels.join('')}</g><rect x="24" y="20" width="410" height="75" rx="8" fill="#f6ebc9" stroke="#6a451c" stroke-width="3"/><text x="45" y="55" class="title">${escAttr(map.name)}</text><text x="45" y="79" class="subtitle">Settlement · ${wardCount} wards · seed ${escAttr(map.seed)}</text><style>.roads path{fill:none;stroke:#e8dfbd;stroke-width:8;stroke-linecap:round}.buildings rect{fill:#7b4b34;stroke:#321d15;stroke-width:1}.labels text{font:800 12px Georgia,serif;paint-order:stroke;stroke:#f4efd9;stroke-width:3px;fill:#2d2115}.title{font:900 27px Georgia,serif;fill:#39220e}.subtitle{font:700 13px sans-serif;fill:#604321}</style></svg>`;
 return {
  svg,markers:[],regions:wardCount,wards,
  geojson:{type:'FeatureCollection',features:wards.map(w=>({
   type:'Feature',properties:{name:w.name,scope:'ward'},
   geometry:{type:'Polygon',coordinates:[[...w.points,w.points[0]].map(p=>[+p[0].toFixed(3),+p[1].toFixed(3)])]}
  }))}
 };
}
