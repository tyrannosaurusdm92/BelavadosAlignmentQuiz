...
(()=>{
  ...
  const appRootUrl=new URL('./',location.href),rootUrl=new URL('./json/tablegate/knowledge-pack/',location.href);
  const urlFor=file=>new URL(file.resolvedPath||file.path,file.resolvedPath?appRootUrl:rootUrl);
  ...
  function renderPreview(){
    ...
    if(UI.preview==null)return `<div class="knowledge-file-summary"><h2>${esc(file.title)}</h2><p>${esc(file.summary||'Structured JSON reference material for '+file.system+'.')}</p><dl><dt>System</dt><dd>${esc(file.system)}</dd><dt>File</dt><dd>${esc(file.path)}</dd><dt>Size</dt><dd>${esc(formatBytes(file.sizeBytes))}</dd><dt>Records</dt><dd>${esc(file.recordCount??'Structured document')}</dd><dt>SHA-256</dt><dd><code>${esc(file.sha256)}</code></dd></dl><div class="row wrap"><button class="primary" data-knowledge-load>Load preview</button><a class="secondary" href="${esc(urlFor(file).href)}" target="_blank" rel="noopener">Open JSON file</a><button data-knowledge-ingest>Ingest into backend knowledge</button></div>${UI.ingestProgress?`<div class="info-box">${esc(UI.ingestProgress)}</div>`:''}</div>`;
  ...
  function render(){
    ...
    return `<div class="workspace-grid knowledge-browser"><section class="workspace-card span-12 knowledge-hero"><div><span class="eyebrow">INJECTED JSON DATASET</span><h2>Nine-System TTRPG Knowledge Pack</h2><p>${catalog.logicalFileCount||catalog.fileCount} indexed JSON references across ${catalog.systemCount} systems are included in this release; ${catalog.physicalKnowledgePackFileCount||catalog.fileCount} live in the knowledge-pack tree and ${catalog.sharedCanonicalReferenceCount||0} use deduplicated shared system files and available to the rules library, backend knowledge ingestion, dice helpers, generators, and campaign preparation tools.</p></div><div class="backend-metrics"><strong>${esc(formatBytes(catalog.totalBytes))}</strong><small>uncompressed JSON</small></div></section>
  ...
  async function load(){const file=selected();if(!file)return;UI.busy=true;UI.preview=null;rerender();try{const response=await fetch(urlFor(file));if(!response.ok)throw new Error(`HTTP ${response.status}`);UI.preview=await response.json()}catch(error){UI.preview={error:'Could not load this JSON file through the current hosting context.',detail:error.message,path:file.path};showError(error)}finally{UI.busy=false;rerender()}}
