...
const DICE = (function() {
    ...
    var vars = { //todo: make these configurable on init
        ...
        desk_color: '#000000', // transparent physics table; visible color removed
        desk_opacity: 0,
        use_shadows: true,
    ...
    that.set_sound_pool = function(pool) {
        vars.sound_pool = Array.isArray(pool) ? pool.filter(function(sound) { return sound && (sound.url || typeof sound === 'string'); }) : [];
    }
    ...
    that.dice_box.prototype.reinit = function(container) {
        ...
        this.desk = new THREE.Mesh(new THREE.PlaneGeometry(this.w * 2, this.h * 2, 1, 1), 
                new THREE.MeshPhongMaterial({ color: vars.desk_color, opacity: vars.desk_opacity, transparent: true, depthWrite: false }));
    ...
    function throw_dices(box, vector, boost, dist, before_roll, after_roll) {
        ...
        let volume = Math.max(0.1, Math.min(1, numDice / 10));
    ...
    function playSound(outerContainer, soundVolume) {
        ...
        var pool = Array.isArray(vars.sound_pool) ? vars.sound_pool : [];
        var picked = pool.length ? pool[Math.floor(Math.random() * pool.length)] : null;
        var src = picked ? (picked.iframeUrl || picked.url || picked) : 'assets/audio/sessions/dice-roller/nc93322.mp3';
        if (current_roll_audio) {
            try { current_roll_audio.pause(); current_roll_audio.remove(); } catch(e) {}
            current_roll_audio = null;
        }
        const audio = document.createElement('audio');
        current_roll_audio = audio;
        ...
        audio.onended = () => {
          if (current_roll_audio === audio) current_roll_audio = null;
          audio.remove();
        };
    }
...
// so the previous build ignored selected style/audio payloads and stayed on fallback colors.
