 ...
 * The parent D&D engine owns rules interpretation, secure random results, Advantage/Disadvantage,
 * modifiers, keep/drop logic, death-save interpretation, and edition-sensitive guidance.
...
var main = (function() {
  ...
  that.init=function(){
    ...
    setResult('<b>Universal 3D dice table ready.</b><br><small>The parent rules engine supplies exact results.</small>');
  ...
  that.rollInput=function(rawPayload){
    ...
    box.start_throw(function(){return requested&&requested.length?requested:null;},function(notation){
      ...
      setResult((displayTotal===null?esc(tableLine):'<b>Result total: '+esc(displayTotal)+'</b>')+'<br><small>3D table: '+esc(tableLine)+'</small>');
  ...
  window.DnDPortalDiceRenderer={roll:function(payload){that.rollInput(payload);}};
  return that;
}());
