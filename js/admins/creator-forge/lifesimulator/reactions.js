(function () {
  ...
  function iconPath(reaction) {
    ...
    if (reaction.namespace && reaction.namespace !== "core") {
      return `assets/svg/admins/creator-forge/reactions/expansions/${encodeURIComponent(reaction.namespace)}/${reaction.icon.split("/").map(encodeURIComponent).join("/")}`;
    }
    return `assets/svg/admins/creator-forge/reactions/core/${reaction.icon.split("/").map(encodeURIComponent).join("/")}`;
  }
