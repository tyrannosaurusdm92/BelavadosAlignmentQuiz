...
function routeRegister_(ctx) {
  ...
  var user = insert_('Users', {
    ...
    disabled:false, discoverable:normalizeAgeBand_(p.ageBand)!=='MINOR', emailVerified:!verificationRequired, emailVerifiedAt:verificationRequired ? '' : now,
  ...
  if (verificationRequired) {
    ...
    var inviteOutcome=invite?registrationInviteOutcome_(invite,user.id):null;
    return {user:privateUser_(user), verificationRequired:true, codeExpiresAt:challenge.row.expiresAt, joinedTablegateId:inviteOutcome&&inviteOutcome.joined?invite.tablegateId:'',inviteOutcome:inviteOutcome};
  }
...
function routeCreateGroupFinderPost_(ctx){
...
function routeExpressGroupFinderInterest_(ctx){
...
function routeCreateRightNowPost_(ctx){ctx.params.postType='RIGHT_NOW';ctx.params.isRightNow=true;return routeCreateGroupFinderPost_(ctx);}
