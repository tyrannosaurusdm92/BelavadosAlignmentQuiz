...
AUDITS = ROOT / "docs" / "github" / "audits" / "reproducible-static"
MANIFESTS = ROOT / "docs" / "github" / "manifests" / "reproducible-static"
...
SELF_EXCLUDED = {
    "docs/previous-project-versions/records/manifests-file-manifest-v10.json",
    "docs/previous-project-versions/records/manifests-code-usage-manifest-v10.json",
    "docs/previous-project-versions/records/audits-automated-test-report-v10.json",
    "docs/previous-project-versions/records/audits-backend-route-coverage-v10.json",
    "docs/previous-project-versions/records/audits-knowledge-pack-audit-v10.json",
}
...
def classify(path: Path, html_loaded: set[str], sw_loaded: set[str]) -> tuple[str, str]:
    ...
    if rel == "backend/api/tablegate-backend-v3.gs":
        return "backend-reference", "Exact unchanged copy of the supplied Apps Script backend"
    if rel.startswith("json/tablegate/knowledge-pack/"):
    ...
    if rel in {"tablegate.html", "json/tablegate/pwa/tablegate.webmanifest", "service-worker.js"}:
...
def main() -> int:
    ...
    backend_path = ROOT / "backend" / "api" / "tablegate-backend-v3.gs"
    ...
    core_text = (ROOT / "js" / "tablegate" / "messaging" / "messenger-core.js").read_text(encoding="utf-8", errors="replace")
    ...
    route_catalog = json.loads((ROOT / "json" / "tablegate" / "backend" / "backend-route-catalog.json").read_text(encoding="utf-8"))
    ...
    excluded_generated = {"js/tablegate/backend/backend-route-catalog.js", "js/tablegate/knowledge/knowledge-pack-catalog.js", "js/tablegate/docs/docs-catalog.js"}
    ...
    knowledge_catalog = json.loads((ROOT / "json" / "tablegate" / "knowledge-pack" / "catalog.json").read_text(encoding="utf-8"))
    ...
    for item in knowledge_files:
        ...
        resolved = str(item.get("resolvedPath", ""))
        path = (ROOT / resolved) if resolved else (ROOT / "json" / "tablegate" / "knowledge-pack" / rel)
        if not path.is_file():
            knowledge_failures.append({"path": rel, "resolvedPath": resolved or None, "error": "missing"})
            continue
    ...
    actual_json_rel = {
        p.relative_to(ROOT / "json" / "tablegate" / "knowledge-pack").as_posix()
        for p in (ROOT / "json" / "tablegate" / "knowledge-pack").rglob("*.json")
        if p.name != "catalog.json"
    }
    unlisted = sorted(actual_json_rel - seen_knowledge_paths)
    physical_catalog_paths = {str(item.get("path", "")) for item in knowledge_files if not item.get("resolvedPath")}
    missing_from_disk = sorted(physical_catalog_paths - actual_json_rel)
    knowledge_ok = (
        ...
        and knowledge_catalog.get("physicalKnowledgePackFileCount") == len(actual_json_rel)
        and knowledge_catalog.get("sharedCanonicalReferenceCount") == len([item for item in knowledge_files if item.get("resolvedPath")])
    )
    add_test(tests, "knowledge catalog resolves 244 unique logical JSON references across nine systems", knowledge_ok, {"catalogFiles": len(knowledge_files), "physicalKnowledgeFiles": len(actual_json_rel), "sharedCanonicalReferences": len([item for item in knowledge_files if item.get("resolvedPath")]), "systems": dict(sorted(systems.items())), "catalogBytes": knowledge_catalog.get("totalBytes"), "actualLogicalBytes": actual_knowledge_bytes, "failures": knowledge_failures, "unlisted": unlisted, "missingFromDisk": missing_from_disk})
    ...
    write_json(AUDITS / "backend-route-coverage.json", route_report)
    ...
    knowledge_report = {
        ...
        "catalogPath": "json/tablegate/knowledge-pack/catalog.json",
        "failures": knowledge_failures,
        "unlisted": unlisted,
        "missingFromDisk": missing_from_disk,
    }
    write_json(AUDITS / "knowledge-pack-audit.json", knowledge_report)
    ...
    report = {
        ...
        "reports": {
            "backendCoverage": "docs/previous-project-versions/records/audits-backend-route-coverage-v10.json",
            "knowledgeAudit": "docs/previous-project-versions/records/audits-knowledge-pack-audit-v10.json",
            "fileManifest": "docs/previous-project-versions/records/manifests-file-manifest-v10.json",
            "codeUsageManifest": "docs/previous-project-versions/records/manifests-code-usage-manifest-v10.json",
            "browserContract": "docs/previous-project-versions/records/audits-browser-contract-test-v10.json",
        },
    }
    write_json(AUDITS / "static-release-test-results.json", report)
