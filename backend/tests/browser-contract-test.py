...
REPORT=ROOT/'docs/previous-project-versions/records/audits-browser-contract-test-v10.json'
...
def main():
    ...
    try:
        with sync_playwright() as pw:
            ...
            for css in ['css/tablegate/messaging/messenger.css','css/tablegate/organizer/organizer.css','css/tablegate/layout/responsive.css','css/admins/backend-center/backend-center.css']:
                page.add_style_tag(content=(ROOT/css).read_text(errors='replace'))
            page.add_script_tag(content=(ROOT/'js/tablegate/messaging/messenger-core.js').read_text(errors='replace'))
            ...
            for js in ['js/tablegate/backend/backend-route-catalog.js','js/tablegate/knowledge/knowledge-pack-catalog.js','js/admins/backend-center/backend-capability-center.js']:
                page.add_script_tag(content=(ROOT/js).read_text(errors='replace'))
            kb=(ROOT/'js/tablegate/knowledge/knowledge-pack-browser.js').read_text(errors='replace').replace("const rootUrl=new URL('./json/tablegate/knowledge-pack/',location.href);","const rootUrl=new URL('https://tablegate.test/json/tablegate/knowledge-pack/');")
