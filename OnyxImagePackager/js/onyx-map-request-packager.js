(() => {
  ...
  const ONYX_MARKER_SVG = `<?xml version="1.0" encoding="utf-8"?>
<svg fill="currentColor" width="800px" height="800px" viewBox="0 0 8 8" xmlns="http://www.w3.org/2000/svg">
...
.map-status{
  ...
  const state = {
    ...
    locationPinTemplate: { fileName: '', data: null, raw: '' },
    packageCandidates: [],
  ...
  async function loadConfig() {
    const results = await Promise.allSettled([
      ...
      fetchJson('json/belavados_locations_pins_template.json')
    ]);
    ...
    if (results[5] && results[5].status === 'fulfilled' && results[5].value) {
      state.locationPinTemplate = { fileName: 'belavados_locations_pins_template.json', data: results[5].value, raw: JSON.stringify(results[5].value, null, 2) };
    }
  }
  ...
  function bindEvents() {
    ...
    if (els.loadBuiltInTemplate) els.loadBuiltInTemplate.addEventListener('click', () => loadBuiltInLocationsPinTemplate(true));
    if (els.locationPinTemplateInput) els.locationPinTemplateInput.addEventListener('change', handleLocationPinTemplateImport);
  ...
  async function loadBuiltInLocationsPinTemplate(userInitiated = false) {
    const data = await fetchJson('json/belavados_locations_pins_template.json');
    if (!data) {
      setTemplateStatus('Built-in locations + pins template could not be loaded. Import a JSON template manually.', true);
      return null;
    }
    state.locationPinTemplate = { fileName: 'belavados_locations_pins_template.json', data, raw: JSON.stringify(data, null, 2) };
    renderTemplatePreview();
    setTemplateStatus(templateSummary(data), false);
    if (userInitiated) addChat('onyx', 'Papa, I loaded the Belavadös immersive locations and color-coded pin template. The marker will change color by the pin hex field, as any civilized marker should.');
    return data;
  }

  function handleLocationPinTemplateImport(e) {
    ...
    reader.onload = () => {
      try {
        ...
        state.locationPinTemplate = { fileName: file.name, data, raw };
        renderTemplatePreview();
        setTemplateStatus(templateSummary(data), false);
        addLog(`Locations + pins template imported: ${file.name}. I shall attach it to the imperial package.`, 'thinking');
      } catch (err) {
        state.locationPinTemplate = { fileName: '', data: null, raw: '' };
        renderTemplatePreview();
        setTemplateStatus('That locations/pins JSON is malformed. Onyx refuses the broken scroll.', true);
      }
    };
  ...
  function setTemplateStatus(text, isError = false) {
    ...
    els.templateStatus.textContent = text || 'No locations + pins template loaded yet.';
    els.templateStatus.classList.toggle('error', !!isError);
    els.templateStatus.classList.toggle('muted', !text || !!isError);
  }

  function templateSummary(data) {
    if (!data) return 'No locations + pins template loaded yet.';
    const profiles = (((data.immersiveLocations || {}).profiles) || []).length || ((data.profileCount) || 0);
    const pinTypes = data.pinsAndGeojson && data.pinsAndGeojson.pinTypes ? Object.values(data.pinsAndGeojson.pinTypes).reduce((sum, list) => sum + (Array.isArray(list) ? list.length : 0), 0) : 0;
    const schema = data.schema || 'locations/pins template';
    return `Loaded ${schema}: ${profiles} immersive location profiles and ${pinTypes} color-coded pin types.`;
  }

  function renderTemplatePreview() {
    const data = state.locationPinTemplate && state.locationPinTemplate.data;
    if (!data) {
      setTemplateStatus('Built-in Belavadös template will auto-load when available.', false);
      if (els.templatePinPreview) els.templatePinPreview.innerHTML = '';
      return;
    }
    setTemplateStatus(templateSummary(data), false);
    if (!els.templatePinPreview) return;
    const pinTypes = (data.pinsAndGeojson && data.pinsAndGeojson.pinTypes) || {};
    const preview = [];
    Object.entries(pinTypes).forEach(([category, list]) => {
      (Array.isArray(list) ? list : []).slice(0, 1).forEach(item => preview.push({ category, pinType: item.pinType || item[0] || 'Location', hex: item.hex || item[1] || '#f3cf78' }));
    });
    els.templatePinPreview.innerHTML = preview.slice(0, 10).map(item => `
      <div class="template-pin-chip" style="--pin-color:${escapeHtml(item.hex)}">
        <span class="marker-icon" aria-hidden="true"></span>
        <span><strong>${escapeHtml(item.pinType)}</strong><small>${escapeHtml(item.category)} • ${escapeHtml(item.hex)}</small></span>
      </div>`).join('');
  }
  ...
  async function buildAndDownloadMapPackage() {
    try {
      ...
      if (!selected.length) {
        addLog('No image assets are selected. I will still package the settlement JSON and locations/pins template, but the map pantry is empty.', 'hungry');
      }
      setPackageStatus('Building ZIP package. Onyx is carefully stuffing the box with useful images and template JSON...', false);
      if (shouldIncludeLocationPinTemplate()) await ensureLocationPinTemplate();
      ...
      if (shouldIncludeLocationPinTemplate() && state.locationPinTemplate && state.locationPinTemplate.data) {
  ...
  function shouldIncludeLocationPinTemplate() {
    return !els.includeTemplateInPackage || !!els.includeTemplateInPackage.checked;
  }

  async function ensureLocationPinTemplate() {
    if (state.locationPinTemplate && state.locationPinTemplate.data) return state.locationPinTemplate.data;
    return await loadBuiltInLocationsPinTemplate(false);
  }

  function buildTemplateManifestSummary() {
    const data = state.locationPinTemplate && state.locationPinTemplate.data;
    if (!data || !shouldIncludeLocationPinTemplate()) return { included: false };
    const profiles = (((data.immersiveLocations || {}).profiles) || []).length || ((data.profileCount) || 0);
    const pinTypes = data.pinsAndGeojson && data.pinsAndGeojson.pinTypes ? Object.values(data.pinsAndGeojson.pinTypes).reduce((sum, list) => sum + (Array.isArray(list) ? list.length : 0), 0) : 0;
    const scaleTargets = data.pinsAndGeojson && data.pinsAndGeojson.settlementScaleTargets ? data.pinsAndGeojson.settlementScaleTargets : {};
    return {
      included: true,
      fileName: state.locationPinTemplate.fileName || 'belavados_locations_pins_template.json',
      schema: data.schema || null,
      immersiveLocationProfiles: profiles,
      colorCodedPinTypes: pinTypes,
      settlementScaleTargets: scaleTargets,
  ...
  function parseChatCommand(text) {
    ...
    if (/(load|import|use|include).*(locations?|pins?|template|marker)/.test(lower)) {
      loadBuiltInLocationsPinTemplate(true);
      response.push('loaded the locations and pins template');
    }
    ...
    if (!response.length) {
      addChat('onyx', pickVoice('chat', 'Yes, Papa. I am listening.') + ' You can also just talk to me, ask about food, say hi buddy, or tell me to fetch map assets, load the locations/pins template, upload settlement JSON, scan a map, promote GeoJSON, or build the ZIP package.');
      return;
    }
